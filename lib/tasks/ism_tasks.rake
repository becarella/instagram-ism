namespace :import do
  ...
  task :instagram_tag, [:tag,:direction, :pages, :ban_filename] => [:environment] do |t, args|
    Ism::Instagram.import_from_tag(args[:tag], :direction => args[:direction], :pages => args[:pages], :ban_filename => args[:ban_filename])
  end
  
  desc "Import Instagram by username"
  task :instagram_user, [:username] => [:environment] do |t, args|
    Ism::Instagram.import_from_user(args[:username])
  end
